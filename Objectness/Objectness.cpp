...
int Objectness::loadTrainedModel(string modelName) // Return -1, 0, or 1 if partial, none, or all loaded
{
	...
	CStr s1 = modelName + ".wS1.yml", s2 = modelName + ".wS2.yml", sI = modelName + ".idx.yml";


	CvMat* myS1;
	myS1 = (CvMat*)cvLoad(s1.data());
	CvMat* myS2;
	myS2 = (CvMat*)cvLoad(s2.data());
	CvMat* mySI;
	mySI = (CvMat*)cvLoad(sI.data());

	Mat filters1f, reW1f, idx1i, show3u;

//m = cv::Mat(&n, true);
	filters1f = cv::Mat(myS1, true);
        idx1i = cv::Mat(mySI, true);
	_svmReW1f = cv::Mat(myS2, true);
	...
	if ( _svmReW1f.size() != Size(2, _svmSzIdxs.size())){
		_svmReW1f = Mat();
		return -1;
	}
	return 1;
